...
export function classify_content(
  ...
  const confidence =
    primaryScore > 0
      ? Math.min(
          1.0,
          primaryScore / (primaryScore + (sortedScores[1]?.[1] || 0) + 1),
        )
      : 0.2;
  return {
...
export function calc_mean_vec(
  ...
  const norm =
    Math.sqrt(wsum.reduce((sum, v) => sum + v * v, 0)) + hybrid_params.epsilon;
  ...
  try {
    ...
    for (const mid of Array.from(ids)) {
      ...
      if (f?.project_id && m.project_id === f.project_id) {
        fs = sigmoid(Math.log(fs / (1 - fs)) + 0.2);
      }
    ...
    for (const r of top) {
      ...
      if (r.path.length > 1) {
        ...
        const pru = await propagateAssociativeReinforcementToLinkedNodes(
          r.id,
          rsal,
          lns,
        );
        for (const u of pru) {
          ...
          if (linked_mem) {
            ...
            const ctx_boost =
              hybrid_params.gamma * (rsal - linked_mem.salience) * decay_fact;
