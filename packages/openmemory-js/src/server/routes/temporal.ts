import { insert_fact, update_fact, invalidate_fact, apply_confidence_decay, get_active_facts_count, get_total_facts_count } from '../../temporal_graph/store'
import { query_facts_at_time, get_current_fact, search_facts, get_facts_by_subject } from '../../temporal_graph/query'
import { get_subject_timeline, get_predicate_timeline, compare_time_points, get_volatile_facts } from '../../temporal_graph/timeline'
...
const create_fact_schema: schema = {
    subject: { type: 'string', required: true, min_length: 1, max_length: 1024 },
    predicate: { type: 'string', required: true, min_length: 1, max_length: 1024 },
...
export const create_temporal_fact = async (req: any, res: any) => {
    ...
    if (!vf.ok) return res.status(400).json({ error: 'invalid valid_from date' })
    const valid_from_date = vf.date ?? new Date()
    const conf = b.confidence !== undefined ? Math.max(0, Math.min(1, b.confidence)) : 1.0

    try {
        const id = await insert_fact(b.subject, b.predicate, b.object, valid_from_date, conf, b.metadata, tenant)
        res.json({
            id,
            subject: b.subject,
...
export const get_temporal_fact = async (req: any, res: any) => {
    ...
    try {
        ...
        const object = typeof req.query.object === 'string' ? req.query.object : undefined
        ...
        if (!at_parsed.ok) return res.status(400).json({ error: 'invalid at date' })
        ...
        if (min_confidence_raw !== undefined && min_confidence_raw !== '') {
            ...
            if (!Number.isFinite(n) || n < 0 || n > 1) {
                return res.status(400).json({ error: 'invalid min_confidence' })
            }
            min_conf = n
        }

        const facts = await query_facts_at_time(subject, predicate, object, at_date, min_conf, tenant)
        res.json({
            facts,
            query: { subject, predicate, object, at: at_date.toISOString(), min_confidence: min_conf, user_id: tenant },
            count: facts.length,
        })
    } catch (error) {
...
export const get_predicate_history = async (req: any, res: any) => {
    ...
    try {
        const predicate = typeof req.query.predicate === 'string' ? req.query.predicate : undefined
        if (!predicate) return res.status(400).json({ error: 'Predicate parameter is required' })

        const from_p = parse_date(req.query.from)
        const to_p = parse_date(req.query.to)
        if (!from_p.ok) return res.status(400).json({ error: 'invalid from date' })
        if (!to_p.ok) return res.status(400).json({ error: 'invalid to date' })

        const timeline_raw = await get_predicate_timeline(predicate, from_p.date, to_p.date)
        const timeline = timeline_raw.filter((entry: any) => {
            const u = entry.fact?.user_id ?? entry.user_id
            return u === undefined || u === null || u === tenant
        })
...
export const update_temporal_fact = async (req: any, res: any) => {
    ...
    const b = parse_or_400<{ confidence?: number; metadata?: Record<string, unknown> }>(
        res,
        req.body,
        update_fact_schema,
    )
    ...
    try {
        ...
        const owned = await query_facts_at_time(undefined, undefined, undefined, new Date(), 0, tenant)
        ...
        const conf = b.confidence !== undefined ? Math.max(0, Math.min(1, b.confidence)) : undefined
        await update_fact(id, conf, b.metadata)
        res.json({ id, confidence: conf, metadata: b.metadata, message: 'Fact updated successfully' })
    } catch (error) {
...
export const invalidate_temporal_fact = async (req: any, res: any) => {
    ...
    const id = req.params.id
    if (!id) return res.status(400).json({ error: 'Fact ID is required' })
    const b = parse_or_400<{ valid_to?: string }>(res, req.body, invalidate_fact_schema)
    if (!b) return
...
export const get_subject_facts = async (req: any, res: any) => {
    ...
    try {
        ...
        const facts_raw = await get_facts_by_subject(subject, at_p.date, include_hist)
...
export const search_temporal_facts = async (req: any, res: any) => {
    ...
    try {
        ...
        const field = typeof req.query.field === 'string' ? req.query.field : 'subject'
        const at_p = parse_date(req.query.at)

        if (!pattern) return res.status(400).json({ error: 'Pattern parameter is required' })
        if (pattern.length > 1024) return res.status(400).json({ error: 'pattern too long' })
        ...
        res.json({ pattern, field, at: at_p.date?.toISOString(), facts, count: facts.length })
    } catch (error) {
    ...
    try {
        ...
        res.json({
            ...
            historical_percentage:
                total_facts > 0 ? ((historical_facts / total_facts) * 100).toFixed(2) + '%' : '0%',
            scope: 'global',
        })
    } catch (error) {
...
export const apply_decay = async (req: any, res: any) => {
    ...
    if (process.env.OM_ADMIN_DECAY !== 'true') {
        return res
            .status(403)
            .json({ error: 'admin_only', message: 'set OM_ADMIN_DECAY=true to enable confidence decay over global facts' })
    }
    const b = parse_or_400<{ decay_rate?: number }>(res, req.body, decay_schema)
    if (!b) return
    const decay_rate = b.decay_rate ?? 0.01

    try {
        const updated = await apply_confidence_decay(decay_rate)
        res.json({ decay_rate, facts_updated: updated, message: 'Confidence decay applied successfully' })
    } catch (error) {
...
export const compare_facts = async (req: any, res: any) => {
    ...
    try {
        ...
        if (!t1_p.ok || !t1_p.date) return res.status(400).json({ error: 'invalid time1' })
        if (!t2_p.ok || !t2_p.date) return res.status(400).json({ error: 'invalid time2' })

        const comparison = await compare_time_points(subject, t1_p.date, t2_p.date)
...
export const get_most_volatile = async (req: any, res: any) => {
    ...
    try {
        ...
        res.json({ subject, limit, volatile_facts: volatile, count: volatile.length })
    } catch (error) {
