...
type q_type = {
  ...
  upd_userSummary: { run: (...p: any[]) => Promise<void> };
...
if (is_pg) {
  ...
  q = {
    ...
    upd_mean_vec: {
      run: (...p) =>
        run_async(`update ${m} set mean_dim=$2,mean_vec=$3 where id=$1`, p),
    },
    upd_compressed_vec: {
      run: (...p) =>
    ...
    get_segment_count: {
      get: (segment) =>
        get_async(`select count(*) as c from ${m} where segment=$1`, [segment]),
    },
    get_max_segment: {
      get: () =>
    ...
    get_waypoint: {
      get: (src, dst) =>
        get_async(`select weight from ${w} where src_id=$1 and dst_id=$2`, [
          src,
          dst,
        ]),
    },
  ...
  q = {
    ...
    get_max_segment: {
      get: () =>
        one("select coalesce(max(segment), 0) as max_seg from memories", []),
    },
    get_segments: {
      all: () =>
