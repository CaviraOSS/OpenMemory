...
export function register_remember_decision_tool(server: mcp_server_sdk, runtime: mcp_runtime): void {
    server.registerTool('openmemory_remember_decision', {
    ...
    }, async (input) => run_audited_tool(runtime, 'openmemory_remember_decision', input, async () => {
        assert_write_allowed(runtime.access, 'openmemory_remember_decision');
