...
export function register_agent_manifest_resource(server: mcp_server_sdk, runtime: mcp_runtime): void {
    server.registerResource('openmemory-agent-memory-manifest', new resource_template('openmemory://project/{project_id}/agent/{agent_id}/manifest', { list: undefined }), {
        description: 'Authenticated, framework-portable OpenMemory asset manifest for one agent', mimeType: 'application/json',
    }, async (uri, values) => {
