...
export const init_command: cli_command = async (context) => {
    ...
    const next_commands = ['openmemory mcp', 'openmemory agent preflight "your task"'];
    if (!context.dry_run) {
    ...
    emit(context, result, () => [
        panel('', context.colors, {
            title: 'OpenMemory initialized', kind: context.dry_run ? 'warning' : 'success', width: context.terminal_width, rows: [
