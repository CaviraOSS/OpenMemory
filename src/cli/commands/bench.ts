 ...
 *  usage : run the packaged Hydrograph benchmark checks
 */
...
export const bench_command: cli_command = async (context) => {
    ...
    if (!passed) throw new cli_error('benchmark_failed', 'One or more benchmark checks failed', exit_codes.benchmark, { checks }, 'openmemory doctor', 'Inspect failed benchmark checks.');
...
export async function run_cli_benchmarks() {
    ...
    try {
        return [
            { name: 'openmemory package surface is available', passed: memory.status().ready },
