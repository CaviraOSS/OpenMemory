...
export const doctor_command: cli_command = async (context) => {
    ...
    const checks = database_exists ? await with_read_memory(context, async (memory) => {
        ...
        return [
            ...
            { check: 'project', status: context.project_id ? 'pass' : 'warn', detail: context.project_name, fix: 'Run openmemory project init' },
            ...
            { check: 'benchmark smoke', status: benchmarks.every((item) => item.passed) ? 'pass' : 'fail', detail: `${benchmarks.filter((item) => item.passed).length}/${benchmarks.length} passed`, fix: 'Run openmemory bench' },
            { check: 'database file', status: existsSync(context.db_path) ? 'pass' : 'warn', detail: context.db_path, fix: 'Run openmemory init' },
        ];
    }) : [
        { check: 'database', status: 'warn', detail: 'not initialized', fix: 'Run openmemory init' },
        { check: 'migrations', status: 'warn', detail: 'not checked', fix: 'Run openmemory init' },
        { check: 'project', status: 'warn', detail: context.project_name, fix: 'Run openmemory init' },
        { check: 'MCP', status: 'pass', detail: 'stdio transport available', fix: '' },
        { check: 'server', status: 'pass', detail: 'server configuration valid', fix: '' },
        { check: 'connectors', status: 'pass', detail: 'connector registry available', fix: '' },
        { check: 'integrity', status: 'warn', detail: 'database not initialized', fix: 'Run openmemory init' },
        { check: 'benchmark smoke', status: benchmarks.every((item) => item.passed) ? 'pass' : 'fail', detail: `${benchmarks.filter((item) => item.passed).length}/${benchmarks.length} passed`, fix: 'Run openmemory bench' },
        { check: 'database file', status: 'warn', detail: context.db_path, fix: 'Run openmemory init' },
    ];
    const ok = checks.every((check) => check.status !== 'fail');
    emit(context, { ok, checks }, () => [
        panel(ok ? 'Core systems are ready.' : 'One or more checks need attention.', context.colors, { title: 'OpenMemory doctor', kind: ok ? 'success' : 'danger', width: context.terminal_width }), '',
