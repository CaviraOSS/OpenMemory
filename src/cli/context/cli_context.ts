...
export type cli_context = {
    ...
    pretty: boolean; compact: boolean; debug: boolean; exit_code: number;
};
...
export function create_cli_context(args: parsed_cli, env: NodeJS.ProcessEnv, io: cli_io): cli_context {
    ...
    const json = has_flag(args, 'json') || !is_tty;
    ...
    return {
        ...
        pretty: has_flag(args, 'pretty') && !has_flag(args, 'compact'), compact: has_flag(args, 'compact'), debug: has_flag(args, 'debug'),
...
export async function with_project<T>(context: cli_context, operation: (project: project_memory) => Promise<T>): Promise<T> {
    ...
    const memory = create_memory({ ...memory_config(context), readonly });
    const project = new project_memory({ memory, tenant_id: 'default', project_id: context.project_id, name: context.project_name, max_context_tokens: context.token_budget });
    try {
