...
export function load_embedding_environment(env: NodeJS.ProcessEnv = process.env): embedding_provider_config | null {
    const selected = value(env, 'OPENMEMORY_EMBEDDING_PROVIDER', 'OM_EMBEDDINGS');
    if (!selected) return null;
    const tier_raw = value(env, 'OPENMEMORY_EMBEDDING_TIER', 'OM_TIER') ?? 'deep';
    ...
    return {
        ...
        fallback: (value(env, 'OPENMEMORY_EMBEDDING_FALLBACK', 'OM_EMBEDDING_FALLBACK') ?? 'synthetic').split(',').map((name) => provider(name.trim(), 'synthetic')),
        tier,
        dimension: number_value(env, ['OPENMEMORY_EMBEDDING_DIMENSION', 'OM_VEC_DIM', 'OM_MAX_VECTOR_DIM'], default_dimension, 1),
        timeout_ms: number_value(env, ['OPENMEMORY_EMBEDDING_TIMEOUT_MS', 'OM_EMBED_TIMEOUT_MS'], 30_000, 1),
        max_retries: number_value(env, ['OPENMEMORY_EMBEDDING_MAX_RETRIES', 'OM_EMBED_MAX_RETRIES'], 2, 0),
        retry_base_ms: number_value(env, ['OPENMEMORY_EMBEDDING_RETRY_BASE_MS'], 250, 0),
        openai_api_key: value(env, 'OPENAI_API_KEY', 'OM_OPENAI_API_KEY'),
        openai_base_url: value(env, 'OPENMEMORY_OPENAI_BASE_URL', 'OM_OPENAI_BASE_URL') ?? 'https://api.openai.com/v1',
        openai_model: value(env, 'OPENMEMORY_OPENAI_EMBEDDING_MODEL', 'OM_OPENAI_MODEL') ?? 'text-embedding-3-small',
        gemini_api_key: value(env, 'GEMINI_API_KEY', 'OM_GEMINI_API_KEY'),
        gemini_base_url: value(env, 'OPENMEMORY_GEMINI_BASE_URL', 'OM_GEMINI_BASE_URL') ?? 'https://generativelanguage.googleapis.com/v1beta',
        gemini_model: value(env, 'OPENMEMORY_GEMINI_EMBEDDING_MODEL', 'OM_GEMINI_MODEL') ?? 'gemini-embedding-001',
        gemini_inputs_per_minute: number_value(env, ['OPENMEMORY_GEMINI_INPUTS_PER_MINUTE'], 0, 0),
        ollama_url: value(env, 'OPENMEMORY_OLLAMA_URL', 'OLLAMA_URL', 'OM_OLLAMA_URL') ?? 'http://127.0.0.1:11434',
        ollama_model: value(env, 'OPENMEMORY_OLLAMA_EMBEDDING_MODEL', 'OM_OLLAMA_MODEL') ?? 'nomic-embed-text',
        aws_region: value(env, 'AWS_REGION', 'AWS_DEFAULT_REGION'),
        aws_model: value(env, 'OPENMEMORY_AWS_EMBEDDING_MODEL', 'OM_AWS_MODEL') ?? 'amazon.titan-embed-text-v2:0',
        siray_api_key: value(env, 'SIRAY_API_TOKEN', 'OM_SIRAY_API_TOKEN'),
        siray_base_url: value(env, 'OPENMEMORY_SIRAY_BASE_URL', 'OM_SIRAY_BASE_URL') ?? 'https://api.siray.ai/v1',
        siray_model: value(env, 'OPENMEMORY_SIRAY_EMBEDDING_MODEL', 'OM_SIRAY_MODEL') ?? 'text-embedding-3-small',
        local_url: value(env, 'OPENMEMORY_LOCAL_EMBEDDING_URL', 'OM_LOCAL_MODEL_URL'),
        local_model: value(env, 'OPENMEMORY_LOCAL_EMBEDDING_MODEL', 'LOCAL_MODEL_PATH', 'OM_LOCAL_MODEL_PATH') ?? 'local-model',
    };
}
