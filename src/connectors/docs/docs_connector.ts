 ...
 *  ----------------------------------------------------------
 *  file  : src/connectors/docs/docs_connector.ts
 *  usage : implements the LongMemory docs connector component
 */
...
import { map_docs_to_hydrograph } from './docs_mapper.js';

export type docs_connector_config = { documents: SourceDocument[]; permission?: connector_permission };

export class docs_connector implements Connector {
    readonly id = 'docs';
    readonly name = 'Drive / Notion-style documents';
    readonly source_type = 'document';
    ...
    async connect(config: connector_config = {}): Promise<void> {
        const options = config as docs_connector_config;
        this.documents.clear();
        for (const document of options.documents ?? []) {
            this.documents.set(document.external_id, { ...document, permissions: options.permission ?? document.permissions ?? public_permission() });
        }
    }
    ...
    async listSources(params: connector_list_params = {}): Promise<SourceRef[]> {
        return [...this.documents.values()].slice(0, params.limit ?? this.documents.size).map((document) => ({
            source_type: document.source_type, external_id: document.external_id, kind: 'document', title: document.title, url: document.url,
            parent_external_id: null, version: document.version, checksum: document.checksum, updated_at: document.updated_at, metadata: document.metadata,
        }));
    }

    async fetchSource(ref: SourceRef): Promise<connector_fetch_result> {
        const document = this.documents.get(ref.external_id);
        if (!document) throw new Error(`document not found: ${ref.external_id}`);
    ...
    async *sync(params: connector_sync_params): AsyncIterable<ConnectorSyncItem> {
        ...
        const seen = new Set(refs.map((ref) => ref.external_id));
        ...
        for (const [external_id, prior] of Object.entries(params.cursor?.items ?? {})) {
            ...
            const ref: SourceRef = { source_type: this.source_type, external_id, kind: 'document', title: external_id, url: null, parent_external_id: null, version: prior.version, checksum: prior.checksum, updated_at: null, metadata: {} };
