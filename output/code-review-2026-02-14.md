...
1. ✅ **D3:** Fix operator precedence bug in title extraction (structured_extraction.ts:269) — **RESOLVED**
2. ✅ **D7:** Fix prohibited_term to report all matches (compliance.ts:351) — **RESOLVED**

### High Priority
3. ✅ **D8:** Batch embedding calls in `store_clauses()` for performance — **RESOLVED**
4. ✅ **Templates/Compliance:** Add ReDoS protection for user-provided regex patterns — **RESOLVED**

### Medium Priority
5. ✅ Standardise UUID generation across codebase — **RESOLVED**
6. ✅ Add version pruning to prevent unbounded growth — **RESOLVED**
...
## Remediation Summary (2026-02-14)

The following fixes were applied to address critical and high-priority issues:

### Critical Fixes
1. **Operator Precedence (structured_extraction.ts:269)**: Added parentheses to correctly group the `&&` expression before `||`:
   ```typescript
   // Before: !first_line.endsWith(".") && first_line === first_line.toUpperCase() || /^[A-Z]/.test(first_line)
   // After:  (!first_line.endsWith(".") && first_line === first_line.toUpperCase()) || /^[A-Z]/.test(first_line)
   ```

2. **Prohibited Term Matching (compliance.ts)**: Changed from single `exec()` to `safe_regex_match_all()` loop to capture all violations.

### High Priority Fixes
3. **Batch Embedding (clause_similarity.ts)**: Refactored `store_clauses()` to compute all embeddings in a single `embed_advanced()` call instead of sequential per-clause API calls.

4. **ReDoS Protection**: Added `src/utils/regex.ts` with:
   - `analyse_regex_safety()` — Detects dangerous patterns (nested quantifiers, etc.)
   - `create_safe_regex()` — Validates patterns before compilation
   - `safe_regex_test()` / `safe_regex_match_all()` — Execution guards with timeout heuristics

### Medium Priority Fixes
5. **UUID Standardisation**: All files now use `rid()` from `src/utils/index.ts` which wraps native `crypto.randomUUID()`. Removed `uuid` package dependency.

6. **Version Pruning**: Added configurable version history limits:
   - `OM_MAX_VERSIONS_PER_MEMORY` env var (default: 50)
   - `prune_versions()` — Per-memory cleanup
   - `prune_all_versions()` — Batch maintenance
   - Auto-pruning on each `save_version()` call

---

## Summary

This is a substantial and well-executed feature expansion. The code follows existing patterns, uses proper TypeScript typing, and integrates cleanly with the existing architecture. ~~The two bugs identified (operator precedence and single-match reporting) should be fixed before production deployment.~~ All critical and high-priority issues have been resolved.
