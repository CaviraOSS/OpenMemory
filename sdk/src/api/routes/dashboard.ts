import { q, all_async, run_async } from "../../database/connection";
...
export function dash(app: any) {
  ...
  app.get("/dashboard/stats", async (req: any, res: any) => {
    try {
      ...
      const err_rate =
        total_reqs > 0 ? ((total_errs / total_reqs) * 100).toFixed(1) : "0.0";
      ...
      const cachit =
        totmem[0]?.count > 0
          ? Math.round(
              (totmem[0].count / (totmem[0].count + total_errs * 2)) * 100,
            )
          : 0;
