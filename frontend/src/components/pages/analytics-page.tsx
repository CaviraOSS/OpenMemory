    ...
    <PageTransition>
        ...
        <StaggerContainer className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StaggerItem>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{avgSalience.toFixed(3)}</div>
                <p className="text-xs text-muted-foreground mt-1">
                  Memory importance
                </p>
              </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Last 10 API requests with token usage</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {apiCalls.slice(-10).reverse().map((call, idx) => (
              <div
                key={idx}
              ...
              >
                ...
                <div className="text-right">
                  <div className="text-sm font-medium">{call.tokens} tokens</div>
          ...
          <CardContent>
            <div className="space-y-3">
              {['query', 'add', 'ingest', 'reinforce'].map(operation => {
                const calls = apiCalls.filter(c => c.endpoint.includes(operation))
