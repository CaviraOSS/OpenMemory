...
export function MemoriesPage() {
  ...
  const filteredAndSortedMemories = useCallback(() => {
    const filtered = memories.filter((memory) => {
      ...
      const matchesSector =
        selectedSector === "all" || memory.primary_sector === selectedSector;
  ...
  const handleAddMemory = async () => {
    ...
    try {
      setIsLoading(true);
      const metadata = newMemory.metadata.trim()
        ? JSON.parse(newMemory.metadata)
        : undefined;
      ...
      toast.success(
        `Memory added successfully (ID: ${result.id}) using ${newMemory.embeddingModel}`,
      );
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        toast.success(
          `Imported ${successCount} of ${imported.length} memories`,
        );
        await loadMemories();
      } catch (error) {
                  ...
                  <div className="grid gap-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        The embedding provider to use (openai, gemini, ollama,
                        local, etc.)
                      </p>
                    </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Find and manage specific memories
                </CardDescription>
              </div>
              <div className="flex gap-2">
                <label htmlFor="import-file" className="hidden">
                  Import JSON file
                </label>
                <input
                  type="file"
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-12">
                      <Checkbox
                        checked={
                          selectedMemories.size === filtered.length &&
                          filtered.length > 0
                        }
