...
export function GraphPage() {
  ...
  const [sortBy, setSortBy] = useState<'salience' | 'date' | 'connections'>('salience')
  ...
  const filteredMemories = memories
    .filter(m => {
      const sectorMatch = filterSector === 'all' || m.primary_sector === filterSector
  ...
  return (
    <PageTransition>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">3D Memory Graph</h2>
      ...
      <div className="flex flex-wrap gap-4 items-center">
        ...
        <Select value={sortBy} onValueChange={(v) => setSortBy(v as 'salience' | 'date' | 'connections')}>
          <SelectTrigger className="w-[180px]">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-sm">X: {rotation.x.toFixed(1)}° Y: {rotation.y.toFixed(1)}°</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="text-sm">{selectedNode ? 'Node selected' : 'None'}</div>
          </CardContent>
        </Card>
      </div>
            ...
            <CardContent className="space-y-2">
              {Object.entries(SECTOR_COLORS).map(([sector]) => (
                <div key={sector} className="flex items-center gap-2">
                  <div className={`w-4 h-4 rounded-full ${SECTOR_COLOR_CLASSES[sector]}`} />
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm">{selectedNode.content.slice(0, 200)}...</p>
                )}
