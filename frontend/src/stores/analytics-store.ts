...
interface AnalyticsState {
  ...
  setSectorDistribution: (
    distribution: Array<{ sector: string; count: number; percentage: number }>,
  ) => void;
...
export const useAnalyticsStore = create<AnalyticsState>((set) => ({
  ...
  setGraphData: (nodes, edges) => set({ graphData: { nodes, edges } }),

  setSectorDistribution: (distribution) =>
    set({ sectorDistribution: distribution }),

  setMemoryGrowth: (growth) => set({ memoryGrowth: growth }),
