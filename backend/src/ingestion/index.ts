...
function splitOversizedParagraph(paragraph: string, sectionSize: number): string[] {
  const trimmed = paragraph.trim();
  if (!trimmed) return [];
  if (trimmed.length <= sectionSize) return [trimmed];
  ...
  for (const rawSentence of sentences) {
    ...
    if (sentence.length >= sectionSize) {
      if (current) {
        chunks.push(current);
        current = '';
      }

      for (let i = 0; i < sentence.length; i += sectionSize) {
        chunks.push(sentence.slice(i, i + sectionSize));
      }
      continue;
    }
    ...
    if (candidate.length > sectionSize && current) {
      chunks.push(current);
      current = sentence;
    } else if (candidate.length > sectionSize) {
...
function splitIntoSections(text: string, sectionSize: number): string[] {
  ...
  for (const para of paragraphs) {
    const chunks = splitOversizedParagraph(para, sectionSize);
    ...
    for (const chunk of chunks) {
      ...
      const needsFlush = currentSection.length > 0 && currentSection.length + trimmedChunk.length + 2 > sectionSize;

      if (needsFlush) {
        sections.push(currentSection.trim());
        currentSection = trimmedChunk;
        continue;
      }
...
async function createRootMemory(originalText: string, extraction: ExtractionResult, metadata?: Record<string, unknown>): Promise<string> {
  const summary = originalText.length > 500 ? originalText.slice(0, 500) + '...' : originalText;

  const rootContent = `[Document: ${extraction.metadata.content_type.toUpperCase()}]\n\n${summary}\n\n[Full content split across ${Math.ceil(
    originalText.length / SECTION_SIZE,
  )} sections]`;
...
async function linkRootToChild(rootId: string, childId: string, sectionIndex: number): Promise<void> {
  ...
  await transaction.begin();
  try {
    await q.ins_waypoint.run(rootId, childId, 1.0, created, created);
    await transaction.commit();
...
export async function ingestDocument(
  ...
  try {
    ...
    for (let i = 0; i < sections.length; i++) {
      try {
        const childId = await createChildMemory(sections[i], i, sections.length, rootId, metadata);
...
export async function ingestURL(url: string, metadata?: Record<string, unknown>, config?: IngestionConfig): Promise<IngestionResult> {
  ...
  try {
    ...
    for (let i = 0; i < sections.length; i++) {
      try {
        const childId = await createChildMemory(sections[i], i, sections.length, rootId, { ...metadata, source_url: url });
        childIds.push(childId);
