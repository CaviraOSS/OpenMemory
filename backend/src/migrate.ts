...
async function get_existing_tables(): Promise<Set<string>> {
    const tables = await all_async(`SELECT name FROM sqlite_master WHERE type='table'`)
    return new Set(tables.map(t => t.name))
}

async function get_existing_indexes(): Promise<Set<string>> {
    const indexes = await all_async(`SELECT name FROM sqlite_master WHERE type='index' AND name NOT LIKE 'sqlite_%'`)
...
async function run_migrations() {
    ...
    console.log(`[MIGRATE] Migration complete: ${created_tables} tables, ${created_indexes} indexes created`)
