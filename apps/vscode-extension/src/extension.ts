...
import { cli_error, openmemory_cli } from './cli.js';
...
export function activate(context: vscode.ExtensionContext): void {
    const output = vscode.window.createOutputChannel('OpenMemory', { log: true });
    const cli = new openmemory_cli(output);
    ...
    const view = vscode.window.createTreeView('openmemory.memories', { treeDataProvider: provider, showCollapseAll: false });
    ...
    const refresh_after_write = async () => {
        ...
        if (vscode.workspace.getConfiguration('openmemory', resource).get<boolean>('autoRefresh', true)) await refresh();
    };
    ...
    context.subscriptions.push(
        ...
        view,
        tracker,
        vscode.commands.registerCommand('openmemory.refresh', () => run('Refreshing OpenMemory', refresh)),
        vscode.commands.registerCommand('openmemory.initialize', async () => {
            const resource = cli.current_resource();
            const result = await run('Initializing OpenMemory', () => cli.run<{ ok: boolean; db_path: string }>(['init'], { resource }));
            if (result) { vscode.window.showInformationMessage(`OpenMemory initialized: ${result.db_path}`); await refresh(); }
        }),
        vscode.commands.registerCommand('openmemory.rememberSelection', async () => {
        ...
        vscode.commands.registerCommand('openmemory.quickNote', async () => {
            const text = await vscode.window.showInputBox({ title: 'Remember a quick note', prompt: 'What should OpenMemory remember?', ignoreFocusOut: true });
            if (!text?.trim()) return;
        ...
        vscode.commands.registerCommand('openmemory.recall', async () => {
            const selected = vscode.window.activeTextEditor?.document.getText(vscode.window.activeTextEditor.selection).trim();
            const query = selected || await vscode.window.showInputBox({ title: 'Recall OpenMemory context', prompt: 'What context do you need?', ignoreFocusOut: true });
            if (!query?.trim()) return;
            const resource = vscode.window.activeTextEditor?.document.uri ?? cli.current_resource();
            const mode = vscode.workspace.getConfiguration('openmemory', resource).get<string>('recallMode', 'associative');
            const result = await run('Recalling context', () => cli.run<recall_result>(['recall', query.trim(), '--mode', mode], { resource }));
            if (result) await show_document('OpenMemory Recall', recall_markdown(result));
        }),
        vscode.commands.registerCommand('openmemory.projectContext', async () => {
            ...
            if (!task?.trim()) return;
            const resource = cli.current_resource();
            const config = vscode.workspace.getConfiguration('openmemory', resource);
            ...
            if (result) {
                ...
                await show_document('OpenMemory Project Context', markdown);
                await vscode.env.clipboard.writeText(markdown);
                vscode.window.showInformationMessage('Project context copied to clipboard.');
            }
        }),
        vscode.commands.registerCommand('openmemory.explain', async (item?: memory_item) => {
        ...
        vscode.commands.registerCommand('openmemory.runDecay', async () => {
        ...
        vscode.commands.registerCommand('openmemory.startAgentSession', () => run('Starting AI change session', () => tracker.start())),
        vscode.commands.registerCommand('openmemory.stopAgentSession', () => run('Stopping AI change session', () => tracker.stop())),
        vscode.commands.registerCommand('openmemory.reviewAgentChanges', () => run('Reviewing AI changes', () => tracker.review())),
        vscode.commands.registerCommand('openmemory.discardAgentChanges', async () => {
        ...
        vscode.commands.registerCommand('openmemory.importSessions', async () => {
            const resource = cli.current_resource();
            ...
            const args = ['port', '--from', source.harness, '--to', 'openmemory', '--agent', agent_id, ...selected.flatMap((item) => ['--id', item.id])];
        ...
        vscode.commands.registerCommand('openmemory.openSettings', () => vscode.commands.executeCommand('workbench.action.openSettings', '@ext:CaviraOSS.openmemory-vscode')),
        vscode.commands.registerCommand('openmemory.showActions', async () => {
            const action = await vscode.window.showQuickPick([
                { label: 'Workspace Memory', kind: vscode.QuickPickItemKind.Separator, command: '' },
                { label: '$(search) Recall context', description: 'Search governed memory', command: 'openmemory.recall' },
                { label: '$(edit) Remember a note', description: 'Store a durable project memory', command: 'openmemory.quickNote' },
                { label: '$(repo) Build project context', description: 'Create an agent-ready coding brief', command: 'openmemory.projectContext' },
                { label: '$(repo-pull) Import AI conversations', description: 'Terminal and code-editor agents', command: 'openmemory.importSessions' },
                { label: 'AI Change Memory', kind: vscode.QuickPickItemKind.Separator, command: '' },
                { label: tracker_status.active ? '$(debug-stop) Stop AI change session' : '$(record) Start AI change session', command: tracker_status.active ? 'openmemory.stopAgentSession' : 'openmemory.startAgentSession' },
                ...(tracker_status.pending ? [{ label: `$(diff) Review ${tracker_status.pending} AI change candidate(s)`, command: 'openmemory.reviewAgentChanges' }] : []),
                { label: 'Management', kind: vscode.QuickPickItemKind.Separator, command: '' },
                { label: '$(refresh) Refresh memories', command: 'openmemory.refresh' },
                { label: '$(database) Initialize workspace', command: 'openmemory.initialize' },
                { label: '$(gear) Open settings', command: 'openmemory.openSettings' },
                { label: '$(output) Open output', command: 'openmemory.openOutput' },
            ], { title: 'OpenMemory actions' });
            if (action?.command) await vscode.commands.executeCommand(action.command);
        }),
        vscode.commands.registerCommand('openmemory.openOutput', () => output.show(true)),
        vscode.workspace.onDidChangeConfiguration((event) => {
            if (event.affectsConfiguration('openmemory')) void refresh();
        }),
