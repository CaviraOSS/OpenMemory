...
| Durable memory lifecycle parity | Issue #10; PR #12 | Already part of `/v1`: create/get/list/update/reinforce/delete/version/audit. Keep compatibility at SDK/CLI level. |
...
These are valuable, but they must be rebuilt on durable `/v1` semantics instead of restored from old code.
...
| Source connector framework | Issues #33, #175, #176; PRs #9, #146 | Rebuild as source events plus extraction candidates under `/v1/ingest`. Start with webhook/HMAC verification, then GitHub/Notion/Google/URL as optional adapters. |
...
| --- | --- | --- |
| Dashboard/frontend | Issues #11, #82, #89, #99, #116, #133; PRs #14, #83, #100, #143, #145, #160, #161, #164, #168 | Historically caused packaging, build, CORS, Docker, and undefined-vector bugs. Rebuild later against `/v1`. |
...
- Durable lifecycle and update/delete behavior: `/v1/memories` routes.
...
- CLI add/query/list/delete against `/v1`.
...
6. Add MCP rebuild design after `/v1` hardening: STDIO, schema, per-request transport, tenant/project, update/delete/list tools.
