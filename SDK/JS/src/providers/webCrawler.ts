...
export class webCrawler_source extends base_source {
  ...
  constructor(user_id?: string, config?: webCrawler_config) {
  ...
  async _list_items(filters: Record<string, any>): Promise<source_item[]> {
    ...
    while (to_visit.length > 0 && this.crawled.length < this.max_pages) {
      ...
      try {
        ...
        console.warn(`[webCrawler] failed to fetch ${url}: ${e.message}`);
      }
    }
  ...
  async _fetch_item(item_id: string): Promise<source_content> {
    ...
    return {
      ...
      text,
      data: text,
      meta: { source: "webCrawler", url: item_id, char_count: text.length },
    };
  }
}
